with users1 as (select time::date as date,
                       user_id,
                       ua.order_id,
                       order_price,
                       ...
                       count(action) filter (where action = 'cancel_order') over (partition by ua.order_id) as is_canceled
                from   user_actions ua join (select distinct order_id,
                                                             sum(price) over (partition BY order_id) as order_price
                                             from   (select order_id,
                                                            unnest(product_ids) as product_id
                                                     from   orders) t join products p using(product_id)) t2
                        ON ua.order_id = t2.order_id)
   
select
    distinct concat('Day ', date - min(date) over (partition by ads_campaign)) as day, 
    concat('Кампания № ', ads_campaign) as ads_campaign,
    round(sum(order_price) over (partition by ads_campaign order by date) / user_count, 2) as cumulative_arppu,
    round(250000.0 / user_count, 2) as cac 
from users1
join (
    select 
        ...
        ads_campaign
    from users1
    where ads_campaign != 0 and user_id in (
        select 
            distinct user_id
        from user_actions 
...
) u using(ads_campaign)
where ads_campaign = 2 and is_canceled = 0 
