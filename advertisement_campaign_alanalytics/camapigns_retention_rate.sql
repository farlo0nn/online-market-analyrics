    ...
    (select time::date as date,
            ...
            case
                when user_id in (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732,
                                 ...
                                 10057, 10064, 10082, 10103, 10105, 10122, 10134, 10135) 
                    then 1
            ...
            count(action) filter (
                                  where action = 'cancel_order') over (partition by ua.order_id) as is_canceled
         ...
         (select distinct order_id,
          ...
          from
              (select order_id,
               ...
               from orders) t
          join products p using(product_id)) t2 on ua.order_id = t2.order_id)
select concat('Campaign № ', ads_campaign) as ads_campaign,
       start_date, date - start_date as day_number,
                          round(count(user_id)::decimal / max(count(user_id)) over (partition by ads_campaign, start_date), 2) as retention
from
    (select distinct user_id,
                     ads_campaign,
                     min(date) over (partition by user_id) as start_date, date
     from campaigns
     where ads_campaign != 0) t1
