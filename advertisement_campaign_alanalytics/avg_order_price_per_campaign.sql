     ...
     join
         (select distinct order_id,
          ...
          join products p using(product_id)) t2 on ua.order_id = t2.order_id
           where  time::date between '2022-09-01'
...
select concat('Campaign № ', ads_campaign) as ads_campaign,
       round(avg(avg_user), 2) as avg_check
from   (select user_id,
               ads_campaign,
               is_canceled,
               avg(order_price) as avg_user
        from   campaigns
        group by user_id, ads_campaign, is_canceled) t3
where  ads_campaign != 0
   and is_canceled = 0
